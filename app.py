...
DAY_SWITCH_PAUSE_MINUTES = int(os.environ.get("DAY_SWITCH_PAUSE_MINUTES", "2"))
MAX_STUDY_DAY = 4
...
def chat_is_expired(chat_history):
    """
    True bedeutet: Die Eingabezeit für diesen Tag ist beendet.
    Der nächste Tag startet aber erst nach der zusätzlichen Pause.
    """
    return get_chat_elapsed_seconds(chat_history) >= CONVERSATION_DURATION_MINUTES * 60


def next_day_is_unlocked(chat_history):
    """
    True bedeutet: Gesprächszeit + Wartepause sind vorbei,
    deshalb darf der nächste Studientag geöffnet werden.
    """
    unlock_after_seconds = (CONVERSATION_DURATION_MINUTES + DAY_SWITCH_PAUSE_MINUTES) * 60
    return get_chat_elapsed_seconds(chat_history) >= unlock_after_seconds
...
def get_active_study_day():
    for day in range(1, MAX_STUDY_DAY + 1):
        ...
        # Während der 5 Minuten Gesprächszeit und während der 2 Minuten Pause
        # bleibt derselbe Tag aktiv. Erst danach wird der nächste Tag geöffnet.
...
def load_chat():
    ...
    try:
        ...
        return jsonify({
            ...
            "pause_seconds": DAY_SWITCH_PAUSE_MINUTES * 60,
...
def send():
    ...
    try:
        ...
        if chat_is_expired(chat_history):
            return jsonify({
                "error": "Die Gesprächszeit für diesen Tag ist abgelaufen. Das nächste Gesprächsthema öffnet sich nach der kurzen Pause automatisch.",
                "expired": True,
                "study_day": study_day,
                "duration_seconds": CONVERSATION_DURATION_MINUTES * 60,
                "pause_seconds": DAY_SWITCH_PAUSE_MINUTES * 60,
